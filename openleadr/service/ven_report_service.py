...
@service('EiReport')
class VENReportService(VENService):
    ...
    async def register_report(self, payload):
        ...
        """
        print('oadrRegisterReport')
        report_requests = []
        args = inspect.signature(self.on_register_report).parameters
        if all(['ven_id' in args,
                'resource_id' in args,
                'measurement' in args,
                'min_sampling_interval' in args,
                'max_sampling_interval' in args,
                'unit' in args,
                'scale' in args]):
            mode = 'compact'
        else:
        ...
        if not 'reports' in payload:
            ...
            response_payload = {'report_requests': [], 'response': {
                'response_code': 200,
                ...
                if mode == 'compact':
                    results = [
                        self.on_register_report(
                            ...
                            max_sampling_interval=rd['sampling_rate']['max_period']) for rd in report['report_descriptions']]
                ...
                if not isinstance(results, list):
                    ...
                    if mode == 'compact':
                        results = [
                             ...
                             *
                             results[i]) for i in range(
                                len(
                                    report['report_descriptions'])) if isinstance(
                                results[i],
                                tuple)]
        ...
        for i, report_request in enumerate(report_requests):
            ...
            sampling_interval = min(
                rrq[2] for rrq in report_request if isinstance(
                    rrq, tuple))
            if not all(
                    rrq is not None and report_request[0][2] == sampling_interval for rrq in report_request):
                logger.error(
        ...
        for i, report_request in enumerate(report_requests):
            ...
            for rrq in report_request:
                ...
                specifier_payloads.append(
                    objects.SpecifierPayload(
                        r_id=r_id, reading_type=reading_type))
    ...
    async def on_register_report(self, payload):
        ...
        """
        logger.warning(
            "You should implement and register your own on_register_report handler "
    ...
    @handler('oadrCreateReport')
    async def create_report(self, request):
        """
        Handle a report that we received from the VTN.
        """
        request_id = request['request_id']
        if 'report_requests' in request:
            report_request_ids = []
            for report_request in request['report_requests']:
            ...
            response_type = 'oadrCreatedReport'
            response_payload = {
                'report_requests': [],
                'pending_reports': report_request_ids,
                'response': {
                    'ven_id': self.ven_id,
                    'response_code': 200,
                    'response_description': 'OK',
    ...
    async def on_update_report(self, payload):
        ...
        """
        logger.warning(
            "You should implement and register your own on_update_report handler "
    ...
    #         logger.info(
    #             f"""VEN {payload['ven_id']} created reports with reportRequestIDs """
    ...
    #             self.created_reports[ven_id].append(
    #                 pending_report['report_request_id'])
    ...
    #             logger.warning(
    #                 f"The requested report with id {requested_report.report_request_id} "
