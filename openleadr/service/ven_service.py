...
class VENService:
    def __init__(self, ven_id):
        ...
        for method in [
            getattr(
                self,
                attr) for attr in dir(self) if callable(
                getattr(
                self,
                attr))]:
    ...
    async def handler(self, request):
        ...
        try:
            ...
            # Pass the message off to the handler and get the response type and
            # payload
            try:
                # Add the request fingerprint to the message so that the
                # handler can check for it.
                # if request.secure and message_type == 'oadrCreatePartyRegistration':
                #     message_payload['fingerprint'] = utils.get_cert_fingerprint_from_request(
                #         request)
                ...
                logger.error(
                    ...
                    f"{err.__class__.__name__}: {err}", exc_info=True)
                raise err
            ...
            logger.warning(
                f"XML schema validation of incoming message failed: {err}.")
            ...
            logger.error(
                f"The VEN server encountered an error: {err.__class__.__name__}: {err}",
                exc_info=True)
    ...
    async def handle_message(self, message_type, message_payload):
        ...
        if message_type in self.handlers:
            ...
            response_type, response_payload = self.error_response(
                'oadrResponse', enums.STATUS_CODES.COMPLIANCE_ERROR, "A message of type "
                f"{message_type} should not be "
                f"sent to this endpoint ({self.__service_name__})")
        logger.info(
            f"Responding to {message_type} with a {response_type} message: {response_payload}.")
